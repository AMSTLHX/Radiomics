...
for k in range(1,11):
    ...
    #robust
    # ror = RobustScaler()
    # X_train.iloc[:,:1647] = ror.fit_transform(X_train.iloc[:,:1647])
    # X_test.iloc[:,:1647] = ror.transform(X_test.iloc[:,:1647])

    # shapeFeat_train = copy.deepcopy(X_train.iloc[:,:17])
    # firstFeat_train = copy.deepcopy(X_train.iloc[:,17:36])
    # GLFeat_train = copy.deepcopy(X_train.iloc[:,36:111])
    # waveletFeat_train = copy.deepcopy(X_train.iloc[:,111:855])
    # squareFeat_train = copy.deepcopy(X_train.iloc[:,855:1040])
    # logFeat_train = copy.deepcopy(X_train.iloc[:,1040:1133])
    # expFeat_train = copy.deepcopy(X_train.iloc[:,1133:1180])
    # laplacianFeat_train = copy.deepcopy(X_train.iloc[:,1180:1645])


    # shapeFeat_test = copy.deepcopy(X_test.iloc[:,:17])
    # firstFeat_test = copy.deepcopy(X_test.iloc[:,17:36])
    # GLFeat_test = copy.deepcopy(X_test.iloc[:,36:111])
    # waveletFeat_test = copy.deepcopy(X_test.iloc[:,111:855])
    # squareFeat_test = copy.deepcopy(X_test.iloc[:,855:1040])
    # logFeat_test = copy.deepcopy(X_test.iloc[:,1040:1133])
    # expFeat_test = copy.deepcopy(X_test.iloc[:,1133:1180])
    # laplacianFeat_test = copy.deepcopy(X_test.iloc[:,1180:1645])
    # clinical_semanticFeat_test = copy.deepcopy(X_test.iloc[:,1645:])
    ...
    smote_enn = SMOTEENN(random_state=seeds)
    ...
    print('clinical_semantic原始特征个数为:{}'.format(clinical_semanticFeat_train.shape[1]))
...
####################标准化######################
    ...
    clinical_semanticFeat_train[:,:2] = ror.fit_transform(clinical_semanticFeat_train[:,:2])
    clinical_semanticFeat_test.iloc[:,:2] = ror.transform(clinical_semanticFeat_test.iloc[:,:2])
...
##################################RFE################
    ...
    rfe = RFE(estimator=LinearSVC(random_state=seeds), n_features_to_select=3)
    rfe = rfe.fit(clinical_semanticFeat_train, y_train)
    clinical_semanticFeat_train = rfe.transform(clinical_semanticFeat_train)
    clinical_semanticFeat_test = rfe.transform(clinical_semanticFeat_test)
    print('train_test_split_seed={} RFE选择cli_sem特征个数为:{}'.format(seeds, clinical_semanticFeat_train.shape[1]))
    radio_and_cliSem_Feat_train = np.hstack((radioFeat_train,clinical_semanticFeat_train))
    radio_and_cliSem_Feat_test = np.hstack((radioFeat_test,clinical_semanticFeat_test))
